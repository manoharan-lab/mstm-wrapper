...
def calc_scat_matrix(targ, inc, th_min, th_max, phi_min, phi_max, delete=True): # targ is target object, inc is incident object
    ...
    print inc.wavelength
    scat_mat_dict = dict.fromkeys(inc.wavelength)
    for wl in inc.wavelength:
        ...
        scat_mat_dict[wl] = [Srow1,th,phi]        
        os.chdir(current_directory)
        if delete:
            shutil.rmtree(temp_dir)

    return scat_mat_dict

def calc_Is(targ, inc, th_min, th_max, phi_min, phi_max):
    scat_mat_dict = calc_scat_matrix(targ, inc, th_min, th_max, phi_min, phi_max)
    Is_dict = dict.fromkeys(inc.wavelength)
    for wl in inc.wavelength:
        Is =[]
        Srow1 = scat_mat_dict[wl][0]
        th = scat_mat_dict[wl][1]
        prefactor = 1/((2*np.pi*targ.index_matrix/wl)**2)
        #calculate Is using scattering matrix
        for i in range(0,len(th)):
            Is.append((Srow1[0][i]*inc.stokes_vec[0] + Srow1[1][i]*inc.stokes_vec[1] + Srow1[2][i]*inc.stokes_vec[2] + Srow1[3][i]*inc.stokes_vec[3])*prefactor*np.sin(th[i]*np.pi/180))
        Is_dict[wl] = Is
    return Is_dict

# class for target which will scatter incident light 
class target:
