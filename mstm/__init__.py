...
def calc_scat_matrix(target, incident, theta, phi, delete=True,
    ...
    modulepath, _ = os.path.split(os.path.abspath(__file__))

    # Search for the executable in the user's path
    # (relies on shutils.which(), which only works in Python 3.3 and above)
    mstmlocation = shutil.which(mstm_executable)
    if mstmlocation is None:
        # search in the path to the current module
        mstmlocation = shutil.which(mstm_executable, path=modulepath)
    if mstmlocation is None:
        raise RuntimeError("MSTM executable" + " \'" + mstm_executable +
                           "\' " + "not found")

    templatelocation = os.path.join(modulepath, 'input_template.txt')
