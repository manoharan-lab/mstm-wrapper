...
def calc_scat_matrix(target, incident, theta, phi, delete=True):
    ...
    for k in range(target.num_spheres):
        g+= '{0:20} {1:20} {2:20} {3:3}'.format(str(radb[k])+'d'+str(rade[k]), str(xb[k])+'d'+str(xe[k]), str(yb[k])+'d'+str(ye[k]), str(zb[k])+'d'+str(ze[k])+'\n')
    with open(templatelocation, 'r') as InFile:
        InF = InFile.read()				
    InF = InF.format(parameters, g, OutputName, angfile_name, len(angs), length_scl_factor_info)
...
if __name__ == "__main__":
    t = Target(np.array([1,1]),np.array([1,1]),np.array([0,1]),np.array([.125,.125]),1.4,1,2)
    inci = Incident((1,0),[1,1,0,0],np.array([0.41,0.4,0.39])) # wavelengths must be in decreasing order 
